...
// Test_identity_services_AuthenticationProfilesAPIService_FetchAuthenticationProfiles tests the FetchAuthenticationProfiles convenience method
func Test_identity_services_AuthenticationProfilesAPIService_FetchAuthenticationProfiles(t *testing.T) {
	...
	testObj := createTestLocalDBFullConfigProfile(t, testName)

	createReq := client.AuthenticationProfilesAPI.CreateAuthenticationProfiles(context.Background()).AuthenticationProfiles(testObj)
	...
	// Cleanup after test
	defer func() {
		deleteReq := client.AuthenticationProfilesAPI.DeleteAuthenticationProfilesByID(context.Background(), *createdID)
		_, _ = deleteReq.Execute()
		t.Logf("Cleaned up test object: %s", *createdID)
	}()

	// Test 1: Fetch existing object by name
	fetchedObj, err := client.AuthenticationProfilesAPI.FetchAuthenticationProfiles(
		context.Background(),
		testName,
	...
	require.NoError(t, err, "Failed to fetch authentication_profiles by name")
	require.NotNil(t, fetchedObj, "Fetched object should not be nil")
	assert.Equal(t, createdID, fetchedObj.Id, "Fetched object ID should match")
	assert.Equal(t, testName, fetchedObj.Name, "Fetched object name should match")
	t.Logf("[SUCCESS] FetchAuthenticationProfiles found object: %s", fetchedObj.Name)

	// Test 2: Fetch non-existent object (should return nil, nil)
	notFound, err := client.AuthenticationProfilesAPI.FetchAuthenticationProfiles(
		context.Background(),
		"non-existent-authentication_profiles-xyz-12345",
		common.StringPtr("Prisma Access"),
		nil,
		nil,
	)
	...
	t.Logf("[SUCCESS] FetchAuthenticationProfiles correctly returned nil for non-existent object")
}
