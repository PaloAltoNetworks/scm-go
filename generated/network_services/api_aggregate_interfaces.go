...
// FetchAggregateInterfaces retrieves a single AggregateInterfaces object by name.
//
...
//
// Returns:
//   - *AggregateInterfaces: The matching object if found, nil otherwise
//   - error: Any error that occurred during the fetch operation
//
// Example:
//
//	obj, err := api.FetchAggregateInterfaces(ctx, "my-object", &folder, nil, nil)
//	if err != nil {
//	    return err
//	}
//	if obj != nil {
...
//	}
func (a *AggregateInterfacesAPIService) FetchAggregateInterfaces(ctx context.Context, name string, folder *string, snippet *string, device *string) (*AggregateInterfaces, error) {
	req := a.ListAggregateInterfaces(ctx).Name(name).Limit(5000)
	...
	if err != nil {
		...
		if httpRes != nil && httpRes.StatusCode == http.StatusOK {
			if apiErr, ok := err.(*GenericOpenAPIError); ok {
				var result AggregateInterfaces
