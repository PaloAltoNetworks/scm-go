...
// Test_security_services_HTTPHeaderProfilesAPIService_Fetch tests the fetch convenience method
func Test_security_services_HTTPHeaderProfilesAPIService_Fetch(t *testing.T) {
	client := SetupSecuritySvcTestClient(t)
	profileName := "test-http-header-fetch-" + common.GenerateRandomString(6)
	profile := security_services.HttpHeaderProfiles{
		Folder: common.StringPtr("Shared"),
		Name:   profileName,
	}
	...
	createRes, _, err := client.HTTPHeaderProfilesAPI.CreateHTTPHeaderProfiles(context.Background()).HttpHeaderProfiles(profile).Execute()
	require.NoError(t, err, "Failed to create HTTP Header Profile for fetch test")
	createdID := *createRes.Id
	require.NotEmpty(t, createdID, "Created profile ID should not be empty")

	// Defer cleanup
	defer func() {
		t.Logf("Cleaning up HTTP Header Profile with ID: %s", createdID)
		_, errDel := client.HTTPHeaderProfilesAPI.DeleteHTTPHeaderProfilesByID(context.Background(), createdID).Execute()
		require.NoError(t, errDel, "Failed to delete profile during cleanup")
	}()

	// Test Fetch by name operation
	fmt.Printf("Attempting to fetch HTTP Header Profile with name: %s\n", profileName)
	fetchedProfile, errFetch := client.HTTPHeaderProfilesAPI.FetchHTTPHeaderProfiles(context.Background(), profileName, common.StringPtr("Shared"), nil, nil)
	...
	assert.Equal(t, "Shared", *fetchedProfile.Folder, "Folder should match")
	t.Logf("Successfully fetched HTTP Header Profile: %s", profileName)

	// Test fetching non-existent profile (should return nil)
	nonExistentName := "non-existent-http-header-xyz-12345"
	notFoundProfile, errNotFound := client.HTTPHeaderProfilesAPI.FetchHTTPHeaderProfiles(context.Background(), nonExistentName, common.StringPtr("Shared"), nil, nil)
