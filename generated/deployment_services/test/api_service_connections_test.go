...
// Test_deployment_services_ServiceConnectionsAPIService_FetchServiceConnections tests the FetchServiceConnections convenience method
func Test_deployment_services_ServiceConnectionsAPIService_FetchServiceConnections(t *testing.T) {
	// Setup the authenticated client
	client := SetupDeploymentSvcTestClient(t)

	// Create test object using same payload as Create test
	randomSuffix := common.GenerateRandomString(6)
	testName := "test-sc-fetch-" + randomSuffix

	// Create dependencies (IPsec Tunnel, Gateway, Crypto Profile)
	tunnelName, cleanupDeps := createSCTestIPsecTunnelAndDeps(t, "sc-fetch-"+randomSuffix)

	testObj := deployment_services.ServiceConnections{
		Name:        testName,
		IpsecTunnel: tunnelName,
		Region:      "us-central1-a",
	}
	testObj.SetOnboardingType("classic")
	testObj.SetSubnets([]string{"10.0.0.0/24", "10.0.1.0/24"})
	testObj.SetSourceNat(true)

	createReq := client.ServiceConnectionsAPI.CreateServiceConnections(context.Background()).ServiceConnections(testObj)
	...
	// Cleanup after test
	defer func() {
		deleteReq := client.ServiceConnectionsAPI.DeleteServiceConnectionsByID(context.Background(), createdID)
	...
	fetchedObj, err := client.ServiceConnectionsAPI.FetchServiceConnections(
		context.Background(),
		testName,
	...
	require.NoError(t, err, "Failed to fetch service_connections by name")
	require.NotNil(t, fetchedObj, "Fetched object should not be nil")
	assert.Equal(t, createdID, fetchedObj.Id, "Fetched object ID should match")
	assert.Equal(t, testName, fetchedObj.Name, "Fetched object name should match")
	t.Logf("[SUCCESS] FetchServiceConnections found object: %s", fetchedObj.Name)

	// Test 2: Fetch non-existent object (should return nil, nil)
	notFound, err := client.ServiceConnectionsAPI.FetchServiceConnections(
		context.Background(),
		"non-existent-service_connections-xyz-12345",
		common.StringPtr("Service Connections"),
		nil,
		nil,
	)
	...
	t.Logf("[SUCCESS] FetchServiceConnections correctly returned nil for non-existent object")
}
