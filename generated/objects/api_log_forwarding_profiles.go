...
// FetchLogForwardingProfiles retrieves a single LogForwardingProfiles object by name.
//
...
//
// Returns:
//   - *LogForwardingProfiles: The matching object if found, nil otherwise
//   - error: Any error that occurred during the fetch operation
//
// Example:
//
//	obj, err := api.FetchLogForwardingProfiles(ctx, "my-object", &folder, nil, nil)
//	if err != nil {
//	    return err
//	}
//	if obj != nil {
...
//	}
func (a *LogForwardingProfilesAPIService) FetchLogForwardingProfiles(ctx context.Context, name string, folder *string, snippet *string, device *string) (*LogForwardingProfiles, error) {
	req := a.ListLogForwardingProfiles(ctx).Name(name).Limit(5000)
	...
	if err != nil {
		...
		if httpRes != nil && httpRes.StatusCode == http.StatusOK {
			if apiErr, ok := err.(*GenericOpenAPIError); ok {
				var result LogForwardingProfiles
