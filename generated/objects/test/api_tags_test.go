...
// Test_objects_TagsAPIService_FetchTags tests the FetchTags convenience method
func Test_objects_TagsAPIService_FetchTags(t *testing.T) {
	// Setup the authenticated client
	client := SetupObjectSvcTestClient(t)

	// Create test object using same payload as Create test
	testName := "test-tag-fetch-" + common.GenerateRandomString(10)
	testObj := objects.Tags{
		Color:    common.StringPtr("Red"),
		Comments: common.StringPtr("Test tag for fetch API testing"),
		Device:   nil,
	...
	createReq := client.TagsAPI.CreateTags(context.Background()).Tags(testObj)
	...
	// Cleanup after test
	defer func() {
		deleteReq := client.TagsAPI.DeleteTagsByID(context.Background(), createdID)
		_, _ = deleteReq.Execute()
		t.Logf("Cleaned up test object: %s", createdID)
	}()

	// Test 1: Fetch existing object by name
	fetchedObj, err := client.TagsAPI.FetchTags(
		context.Background(),
		testName,
	...
	require.NoError(t, err, "Failed to fetch tags by name")
	require.NotNil(t, fetchedObj, "Fetched object should not be nil")
	assert.Equal(t, createdID, *fetchedObj.Id, "Fetched object ID should match")
	assert.Equal(t, testName, fetchedObj.Name, "Fetched object name should match")
	t.Logf("[SUCCESS] FetchTags found object: %s", fetchedObj.Name)

	// Test 2: Fetch non-existent object (should return nil, nil)
	notFound, err := client.TagsAPI.FetchTags(
		context.Background(),
		"non-existent-tags-xyz-12345",
		common.StringPtr("Prisma Access"),
		nil,
		nil,
	)
	...
	t.Logf("[SUCCESS] FetchTags correctly returned nil for non-existent object")
}
