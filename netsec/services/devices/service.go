...
import (
	...
	mhJDwSQ "github.com/paloaltonetworks/scm-go/netsec/schemas/devices"
)
...
Param Serial (string, required): the Serial param.
*/
...
func (c *Client) Read(ctx context.Context, input ReadInput) (mhJDwSQ.Config, error) {
	...
	// Path param handling.
	path = strings.ReplaceAll(path, "{serial}", input.Serial)
