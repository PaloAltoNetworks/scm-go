...
type CreateInput struct {
	...
	Request *hhIWLbI.Config `json:"request,omitempty"`
...
Param Data ([]hhIWLbI.Config): the Data param.
...
type ListOutput struct {
	Data   []hhIWLbI.Config `json:"data,omitempty"`
	Limit  *int64           `json:"limit,omitempty"`
	Offset *int64           `json:"offset,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var err error
	var ans ListOutput
	var items map[string]hhIWLbI.Config
	maxLimit := int64(2000)
	...
	for {
		...
		items = make(map[string]hhIWLbI.Config)
		total := int(*ans.Total)
	...
	listing := make([]hhIWLbI.Config, 0, len(items))
	for key := range items {
