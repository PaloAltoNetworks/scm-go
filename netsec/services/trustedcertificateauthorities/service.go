...
import (
	...
	iYmUVvF "github.com/paloaltonetworks/scm-go/netsec/schemas/trusted/certificate/authorities"
)
...
Param Data ([]iYmUVvF.Config): the Data param.
...
type ListOutput struct {
	Data   []iYmUVvF.Config `json:"data,omitempty"`
	Limit  *int64           `json:"limit,omitempty"`
	Offset *int64           `json:"offset,omitempty"`
...
//
// Method: get
// URI: /trusted-certificate-authorities
func (c *Client) List(ctx context.Context, input ListInput) (ListOutput, error) {
	// Variables.
	var err error
	var ans ListOutput
	path := "/trusted-certificate-authorities"
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var err error
	var ans ListOutput
	var items map[string]iYmUVvF.Config
	maxLimit := int64(2000)
	...
	for {
		...
		items = make(map[string]iYmUVvF.Config)
		total := int(*ans.Total)
	...
	listing := make([]iYmUVvF.Config, 0, len(items))
	for key := range items {
