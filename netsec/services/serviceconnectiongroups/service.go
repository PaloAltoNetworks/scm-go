package serviceconnectiongroups
...
Param Data ([]sPPuDTU.Config): the Data param.
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var err error
	var ans ListOutput
	var items map[string]sPPuDTU.Config
	maxLimit := int64(2000)
	...
	for {
		...
		items = make(map[string]sPPuDTU.Config)
		total := int(*ans.Total)
	...
	listing := make([]sPPuDTU.Config, 0, len(items))
	for key := range items {
