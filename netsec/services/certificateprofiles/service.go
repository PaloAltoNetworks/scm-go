...
type CreateInput struct {
	...
	Request *dEowMJz.Config `json:"request,omitempty"`
...
Param Data ([]dEowMJz.Config): the Data param.
...
type ListOutput struct {
	Data   []dEowMJz.Config `json:"data,omitempty"`
	Limit  *int64           `json:"limit,omitempty"`
	Offset *int64           `json:"offset,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var err error
	var ans ListOutput
	var items map[string]dEowMJz.Config
	maxLimit := int64(2000)
	...
	for {
		...
		items = make(map[string]dEowMJz.Config)
		total := int(*ans.Total)
	...
	listing := make([]dEowMJz.Config, 0, len(items))
	for key := range items {
