...
type CreateInput struct {
	...
	Request *xlJKoIe.Config `json:"request,omitempty"`
...
Param Data ([]xlJKoIe.Config): the Data param.
...
type ListOutput struct {
	Data   []xlJKoIe.Config `json:"data,omitempty"`
	Limit  *int64           `json:"limit,omitempty"`
	Offset *int64           `json:"offset,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var err error
	var ans ListOutput
	var items map[string]xlJKoIe.Config
	maxLimit := int64(2000)
	...
	for {
		...
		items = make(map[string]xlJKoIe.Config)
		total := int(*ans.Total)
	...
	listing := make([]xlJKoIe.Config, 0, len(items))
	for key := range items {
