...
type CreateInput struct {
	...
	Request *quvWIWu.Config `json:"request,omitempty"`
...
Param Data ([]quvWIWu.Config): the Data param.
...
type ListOutput struct {
	Data   []quvWIWu.Config `json:"data,omitempty"`
	Limit  *int64           `json:"limit,omitempty"`
	Offset *int64           `json:"offset,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var err error
	var ans ListOutput
	var items map[string]quvWIWu.Config
	maxLimit := int64(2000)
	...
	for {
		...
		items = make(map[string]quvWIWu.Config)
		total := int(*ans.Total)
	...
	listing := make([]quvWIWu.Config, 0, len(items))
	for key := range items {
