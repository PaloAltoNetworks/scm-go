...
type CreateInput struct {
	...
	Request *uAgupQd.Config `json:"request,omitempty"`
...
Param Data ([]uAgupQd.Config): the Data param.
...
type ListOutput struct {
	Data   []uAgupQd.Config `json:"data,omitempty"`
	Limit  *int64           `json:"limit,omitempty"`
	Offset *int64           `json:"offset,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var err error
	var ans ListOutput
	var items map[string]uAgupQd.Config
	maxLimit := int64(2000)
	...
	for {
		...
		items = make(map[string]uAgupQd.Config)
		total := int(*ans.Total)
	...
	listing := make([]uAgupQd.Config, 0, len(items))
	for key := range items {
