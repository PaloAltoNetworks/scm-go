...
Param Folder (string, required): the Folder param. String can either be a specific string(`"Remote Networks"`) or match this regex: `^[\s0-9a-zA-Z._-]{1,}$`. Default: `"Remote Networks"`.
...
Param Data ([]uewNibC.Config): the Data param.
...
type ListOutput struct {
	Data   []uewNibC.Config `json:"data,omitempty"`
	Limit  *int64           `json:"limit,omitempty"`
	Offset *int64           `json:"offset,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var err error
	var ans ListOutput
	var items map[string]uewNibC.Config
	maxLimit := int64(2000)
	...
	for {
		...
		items = make(map[string]uewNibC.Config)
		total := int(*ans.Total)
	...
	listing := make([]uewNibC.Config, 0, len(items))
	for key := range items {
